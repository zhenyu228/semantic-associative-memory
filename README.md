...
当前项目正在从“问答数据集驱动”升级为“对象图驱动”。新的框架把一篇论文、一个代码仓库或一个文档集合都视为知识对象：每个对象内部先建立局部证据图，不同对象之间再通过标准化实体建立跨对象桥接边。科研论文集合是第一类落地场景，代码仓库等复杂知识对象可以作为后续扩展场景。
...
- LLM 使用边界：底层建边默认不调用 LLM，主要使用 embedding 相似度、上下文路径、结构位置和关键词/实体重叠；GPT-5.4 主要用于高层语义压缩、查询规划、证据筛选和答案生成。`RelationJudge` 保留为小规模诊断和消融能力，不作为主方法默认建边模块。
...
- CAM 口径在线插入时间实验：`run_cam_style_insertion_experiment.py` 可以复用 CAM Figure 3(a) 的 batch insertion time 口径，对比 GraphRAG/RAPTOR/CAM 离线参考线与 SAM 在线写入、非 LLM 局部建图、GPT-5.2 高层语义压缩耗时，实验说明见 `docs/cam_style_insertion_experiment.md`。
- Bad case 分析：每次实验自动输出 `bad_cases.json` 和 `bad_cases.md`，记录失败类型、诊断和架构改进建议。
- 数据集统一格式：外部数据集先转换成 `sam-dataset-v1`，核心系统不直接依赖 HotpotQA、NovelQA 或 SciFact 原始格式。
...
- 通用对象图框架：`ObjectGraphBuilder` 可以把论文、代码仓库等知识对象转成局部证据图；`EntityBridgeIndex` 会根据方法、数据集、指标、代码符号等桥接实体建立跨对象边；`CrossGraphRetriever` 支持“对象内定位 -> 跨对象跳转 -> 对象内补证据”的联想检索。

## 系统框架

```mermaid
flowchart TD
    A["公开数据集 HotpotQA / NovelQA / SciFact"] --> B["Dataset Adapter 数据集适配层"]
    ...
    E --> F
    A2["知识对象<br/>论文 / 代码仓库 / 文档集合"] --> B2["ObjectGraphBuilder 对象图构建"]
    B2 --> C2["LocalEvidenceGraph 对象内局部图"]
    C2 --> D2["EntityBridgeIndex 跨对象实体桥"]
    D2 --> R2["CrossGraphRetriever 跨图联想检索"]
    R2 --> F
...
SAM/
├── src/sam/
│   ├── datasets.py        # 公开数据集适配，包括 HotpotQA、NovelQA 和 SciFact
...
│   ├── object_graph.py    # 通用对象图、跨对象实体桥和跨图检索框架
...
通用对象图框架 demo 不执行评测，只用于检查“每个对象一个局部图、对象之间通过实体桥连接、查询时返回跨图路径”的框架能力：

```bash
conda run -n sam python scripts/run_object_graph_demo.py --reset
```

输出目录为 `outputs/object_graph_demo/`，其中 `nodes.json`、`edges.json` 和 `retrieval_hits.json` 可以检查对象内边、跨对象实体桥和检索路径。
...
如果公司在线 embedding endpoint 持续超时，可以切换到本地 `sentence-transformers` provider。该依赖是可选依赖，不会随默认安装强制安装：
...
export SAM_SENTENCE_TRANSFORMER_NORMALIZE="1"
...
运行实验时把 provider 改成 `sentence_transformers` 即可，仍建议打开 embedding cache：
...
默认主流程不使用 GPT-5.4 做底层建边。若要做关系噪声诊断或小规模消融，可以启用 GPT-5.4 对候选语义边进行关系级判别。先配置聊天模型环境变量，然后在实验命令中增加 `--relation-judge cached_gpt54`。该模式会把判别结果缓存到 `outputs/cache/relation_judge_cache.json`，避免同一候选边在多轮实验中重复调用模型：
...
`azure_openai_sdk` 遇到 429、RateLimit 或 qpm limit 时会按 `SAM_AZURE_CHAT_RATE_LIMIT_RETRIES` 重试，并用 `SAM_AZURE_CHAT_RATE_LIMIT_SLEEP_SECONDS` 控制最小等待时间。低额度环境中建议设置 `SAM_AZURE_CHAT_MIN_INTERVAL_SECONDS`，让连续生成请求之间保持固定间隔。
...
SciFact 是科学事实核验数据集，包含 scientific claim、科学论文摘要语料、gold evidence abstract 和 rationale sentence。它比 HotpotQA 更接近“跨论文科研检索”场景，适合用来评估不同非 LLM 建图策略的效果和成本。

下载并转换 SciFact dev 样本：
