...
- 类比推理初版：`AnalogyEngine` 可以按历史案例检索相似记忆，并生成可注入 LLM 的类比提示。
- 多智能体共享记忆初版：`SharedMemoryCoordinator` 提供全局洞察层、会话层和交互细节层的统一写入与查询接口。
- 检索-生成闭环初版：`ContextAnswerGenerator` 可以基于 `cases.json` 中的检索上下文调用聊天模型生成答案。
...
│   ├── agents.py          # 多智能体共享记忆最小接口
│   ├── analogy.py         # 类比推理案例检索与提示生成
