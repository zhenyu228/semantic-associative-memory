...
- `AnalogyEngine`：把同一任务或同一查询上下文下的记忆组织为历史案例，综合查询语义相似度、关键词重叠和案例内部关系类型，返回可类比的历史案例，并生成可注入 LLM 的提示文本。当前版本是案例式类比检索，后续需要升级为更严格的路径结构匹配和子图相似度计算。
- `SharedMemoryCoordinator`：提供多智能体共享记忆接口，将记忆分为 `global_insight`、`session`、`interaction` 三层，分别对应开题报告中的全局洞察层、会话层和交互细节层。不同智能体可以向共享记忆写入内容，也可以按层级和会话查询相关记忆。
- `AzureOpenAIEmbeddingProvider`：支持通过环境变量接入 Azure OpenAI 兼容 embedding 服务，使后续实验可以从本地哈希 embedding 切换到正式语义表示模型。

当前 P6 仍是接口和原型阶段，尚未完成正式实验。后续需要增加：
