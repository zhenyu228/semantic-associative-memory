...
在新的方法边界中，底层建边默认不调用 LLM。`GraphBuilder` 的主路径依赖 embedding 相似度、上下文路径邻近度、结构位置、关键词和实体重叠等可复现信号，以保证在线插入和局部建图成本可控。系统保留关系级判别接口 `RelationJudge`，但它不再作为 SAM 主方法的默认建边模块，而用于关系噪声诊断、小规模高风险边过滤消融和成本收益分析。若实验中显式启用 `--relation-judge cached_gpt54`，系统会把判别结果写入 `outputs/cache/relation_judge_cache.json`，并在实验报告中单独说明该结果属于 LLM 辅助过滤设置。

SAM 对 LLM 的正式定位是：底层图关系构建不用 LLM，高层语义压缩和查询阶段可以使用 LLM。高层语义压缩包括把多个底层 memory items 聚合为 summary memory、claim/evidence 摘要或研究主题摘要；查询阶段包括 query planning、证据筛选、冲突裁决、最终回答生成和 grounded answer judging。这样既保留大模型的语义抽象能力，又避免节点两两组合带来的大模型调用成本。
...
当前系统已经进一步加入三层记忆重构机制。第一层是原始底层证据节点，保存公开数据集或长文本切分后的原始语义片段。第二层是单次巩固记忆：反馈模块发现某次 SAM 检索命中支持证据后，`MemoryConsolidator` 会生成一个 `consolidated_memory` 长期记忆节点，把“问题、标准答案、检索方法、答案状态、支持证据摘要”沉淀为新的记忆单元。该节点通过 `consolidates_support` 边连接到原始支持证据，原始证据节点也会记录 `consolidated_by` 和 `consolidation_count`，表示它已经被某次成功推理吸收为长期经验。

第三层是高层洞察记忆：当同一数据集或同一主题下积累了多个 `consolidated_memory` 后，`InsightMemoryBuilder` 会把这些长期记忆重构为 `insight_memory` 节点。洞察节点记录来源巩固记忆、覆盖问题、共享关键词、底层证据集合和可回溯证据标题，并通过 `insight_summarizes_memory` 边连接到巩固记忆，通过 `insight_traces_evidence` 边连接到底层证据。这样系统不只是保存节点和边，而是形成“底层证据 -> 单次巩固记忆 -> 高层洞察记忆”的可检查演化结构。

这一步对应开题报告中的“记忆重构”：系统不只记录访问次数，也会把被验证有效的证据链重写为更高层次的经验节点，再把多个经验节点组织成可复用的高层洞察。后续连续任务可以先命中洞察节点，再回溯到底层证据，而不是每次都从原始段落重新组合证据。

巩固节点和洞察节点在检索中的角色是“中间记忆”，不是最终证据。SAM 方法会把已有 `consolidated_memory` 和 `insight_memory` 加入候选池，使其可被向量激活或图路径经过；最终返回 top-k 时会过滤这些中间记忆节点，避免摘要化经验替代原始证据。这样既能利用历史经验，也能保持评测中的证据可追溯性。
