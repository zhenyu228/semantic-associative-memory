...
跨论文场景中，优先选择带有标准检索答案的数据集，而不是只拿一批论文做无监督可视化。可用数据集分为三类：

1. SciFact / SciFact-Open：给定科学 claim，从论文摘要库中找支持或反驳证据。优点是有 gold evidence，适合直接评估 evidence recall、MRR 和 nDCG；当前代码已经支持 SciFact 小样本转换和实验。
2. LitSearch：给定科研检索问题，目标是找回相关论文。它更贴近“用户问一个科研点，系统跨论文分析”的真实用法，适合后续作为主实验数据。
3. BEIR SciDocs / S2ORC：适合大规模论文检索或建图成本压力测试。SciDocs 有 qrels，可做检索效果评估；S2ORC 更适合测试大规模建图时间、候选对数量和图规模，但单独使用时需要额外任务标签。

因此，本实验中“效果”不定义为边是否看起来合理，而定义为：在相同检索预算下，某种建图策略是否提升 gold evidence 或相关论文的召回和排序质量。
...
对于 SciFact 这类 abstract-only 跨论文数据，`section=abstract` 是全语料共享字段，不能作为跨论文父路径，否则任意两篇论文都会因为同为摘要而获得虚假的结构邻近。因此脚本在 `source_id` 只是文档自身 ID 时，不把 `section:abstract` 写入 `context_path`，只保留标题级路径。这样如果数据本身没有章节、段落或上级对象结构，`context_path_only` 不会被人为抬高。
...
判断策略是否“性价比更高”时，不只看一个指标。最低要求是同时查看：

1. 效果：`evidence_recall`、`mrr`、`ndcg_at_k`、`graph_path_evidence_recall`。
2. 成本：`candidate_pair_count`、`edge_count`、`build_time_seconds`、`total_time_seconds`。
3. 单位成本收益：`recall_per_100_edges`、`recall_per_second`、`gain_per_100_extra_edges`、`gain_per_extra_second`。
4. 综合折中：`cost_index` 和 `cost_effectiveness_score`。

如果某个策略只增加边数和耗时，但没有提升 recall、MRR 或 nDCG，则不能称为更优策略。报告中的 `recommended_strategy` 会按带成本惩罚的 `balanced_score` 给出建议；如果所有图策略都不优于 `no_graph`，结果会明确写为 `no_improving_graph_strategy`。
...
1. 哪种策略效果最好：看 `evidence_recall`，再看 `mrr` 和 `ndcg_at_k` 判断排序质量。
2. 哪种策略最省：看 `candidate_pair_count`、`edge_count` 和 `build_time_seconds`。
3. 哪种策略性价比最高：看 `cost_effectiveness_score`、`balanced_score` 和单位成本收益。
4. 图是否真的贡献了检索：看 `graph_path_support_hits`、`graph_path_evidence_recall` 和相对 `no_graph` 的增益。

在跨论文数据上，如果 `context_path_only` 结果很弱，这不一定说明路径思想无效，也可能说明当前数据只有摘要级文本，没有可利用的论文内部层级。此时应将结论限定为：“在 abstract-only 语料中，结构路径信号不足，主要有效信号来自语义相似；要验证上下文路径，需要 LitSearch、QASPER 全文或自建论文全文切分数据。”这比强行把 section、query 分组或 candidate 集合作为路径更可靠。
