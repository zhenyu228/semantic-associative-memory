...
类比推理目前作为扩展创新点推进。系统已具备长期记忆节点、历史案例检索、关系路径模式匹配和类比提示生成能力，可以在连续任务中把已验证的证据链作为历史经验重新检索出来。
...
- `AnalogyEngine` 会把同一问题上下文中的记忆节点组织为历史案例，综合语义相似、关键词重叠、关系类型和关系路径模式进行排序。
- `MemoryConsolidator` 会把成功检索的证据链沉淀为 `consolidated_memory`，类比模块可以读取这些历史证据链。
- `outputs/runs/analogy_reuse_hotpotqa30_real_embedding_v2/` 完成 30 条 HotpotQA masked probe 类比复用实验，来源案例命中率、巩固案例命中率、支持证据重叠命中率和结构路径匹配率均为 1.000。
- `analogy_reuse_results.md` 已输出 3 个可解释类比命中案例，包含匹配关系路径、支持证据标题和类比提示摘要；本次 30 条没有失败案例，失败案例段落保留用于后续更难设置。

阶段结论：类比推理已经形成“成功检索 -> 记忆巩固 -> 历史案例匹配 -> 类比提示”的受控实验闭环，可以作为扩展创新点展示。下一步需要接入 GPT-5.4 做生成式对照，验证类比提示是否进一步改善最终答案组织和证据引用质量。

## 当前优先级

第一优先级是继续巩固 HotpotQA 300 条主实验：围绕一跳联想的成功结果补充典型案例，并进一步优化二跳路径的噪声控制。第二优先级是把连续记忆复用和类比复用扩展到更自然的多轮任务设置。第三优先级是针对 NovelQA 长文本场景单独优化实体消歧、关系判别和 chunk 定位；真实 embedding 的 NovelQA 12 条 smoke 已经跑通，但 SAM-full 暂未超过 GraphRAG 和 no-graph。第四优先级是继续扩大 GPT-5.4 多智能体生成对照，观察共享记忆和类比提示是否能在更大样本上带来稳定增益。
