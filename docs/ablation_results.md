...
2026-06-11 使用真实 embedding cache 重跑类比复用实验，run 位于 `outputs/runs/analogy_reuse_hotpotqa30_real_embedding_v2/`。该 run 使用 HotpotQA 300 条统一数据文件中的前 30 条，warmup 后构造 masked probe，并在类比检索时自动抽取来源案例的关系路径模式。参数为 `top-k=3`、`retrieval-top-k=4`、`seed-k=1`、`hops=1`。
...
该 run 的 `analogy_reuse_results.md` 新增“类比命中案例”和“类比失败案例”两部分。命中案例会列出当前 probe、Top-1 历史案例、匹配关系路径、支持证据标题和可注入生成阶段的类比提示摘要；本次 30 条均命中对应巩固案例，失败案例分布为 `{"success": 30}`。这说明在受控 masked probe 设置下，系统可以把 warmup 阶段验证过的证据链沉淀为历史经验，并通过关系路径模式在后续问题中重新找回。
...
## 36. GPT-5.4 检索-生成闭环与 Grounding 评测

为推进“检索-生成-反馈闭环”，系统使用 GPT-5.4 运行了低额度端到端生成实验。实验入口为 `scripts/run_end_to_end_experiment.py`，使用 HotpotQA 前 3 条样本、local embedding、`embedding_topk,sam_full` 检索方法、`sam_full` 作为生成上下文，聊天模型使用 `azure_openai_sdk`。

第一轮 run 位于 `outputs/runs/e2e_gpt54_generation_q3/`。SAM-full 的检索证据召回率为 0.667，生成答案命中率为 0.333。bad case 显示两类问题：第一类是检索上下文缺少答案证据，生成阶段无法可靠回答；第二类是检索上下文中存在部分答案线索，但提示词过于保守，GPT-5.4 输出“证据不足”。

据此调整生成提示词：要求模型先逐条检查上下文是否包含问题所问的实体、地点、时间、职位或数量；若存在答案线索，优先抽取最短答案短语，不要过早回答“证据不足”。提示词修正后的 run 位于 `outputs/runs/e2e_gpt54_generation_q3_prompt_v2/`，表面生成答案命中率从 0.333 提升到 0.667。

进一步核查后发现，其中一条命中来自 GPT-5.4 外部知识：模型生成了 `Chief of Protocol`，但检索上下文只包含 `Kiss and Tell` 与 `Shirley Temple`，并没有包含 `Chief of Protocol`。因此系统新增 grounding gate：生成答案必须同时满足“生成答案匹配标准答案”和“检索上下文覆盖答案线索”，才计为命中；如果模型答对但上下文不支持，则标记为 `ungrounded_generated_answer`，不计入最终生成命中。

严格 grounding 后的 run 位于 `outputs/runs/e2e_gpt54_generation_q3_grounded_v2/`：

| 项目 | 数值 |
| --- | ---: |
| 查询数量 | 3 |
| SAM-full 证据召回率 | 0.667 |
| 生成答案命中数 | 1 |
| 生成答案命中率 | 0.333 |
| Ungrounded 命中数 | 1 |
