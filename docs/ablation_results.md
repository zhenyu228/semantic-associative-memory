...
本轮指标与改造前基本一致，原因是该 30 条样本中实际创建的 392 条边均被判定为正常边，没有触发低信息关键词过滤。这个结果说明该改造更像是边质量的安全约束和回归防护，尚未解决主要性能瓶颈。下一阶段需要更强的关系有效性判断：一方面加入实体类型和关系短语约束，另一方面使用 GPT-5.4 对候选边进行语义关系判别，把“是否应该连边”从简单词面规则推进到关系级判断。
